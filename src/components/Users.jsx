...
const Users = ({ setLoading }) => {
  ...
  const getProfileImage = (user, index) => {
    if (index < 10) {
      return profile; 
    } else if (user.avatar) {
      return user.avatar;
    } else {
      return `https://via.placeholder.com/150?text=${user.profile.firstName.charAt(0)}`;
    }
  };
          ...
          {users.length > 0 ? (
            users.map((user, index) => (
              <div key={user.profile.username} onClick={() => userHandler(user.profile.username)}>
                <img src={getProfileImage(user, index)} alt="profile" onError={(e) => (e.target.src = profile)} />
      ...
      <div className={`user-details ${isDetailVisible ? 'isDetailVisible' : ''} ${isSlideIn ? 'isSlideIn' : ''}`}>
        <div className="details-head">
          ...
          <div className="cancel-detail">
            <CancelOutlinedIcon onClick={detailHandler} />
          </div>
        </div>
        <div className="details-body">
          {selectedUser ? (
            <>
              <img src={selectedUser.avatar || profile} alt="profile" onError={(e) => (e.target.src = profile)} />
